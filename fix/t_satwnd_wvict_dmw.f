...
#  {Channel 8 (6.15 um)} (GOES-18/19 satellites)
# ------------------------------------------------------------------------
      eval ORBITLIST${n}=\${ORBITLIST${n}:-goesDMW.wvimgct.list}
      eval TRANSFER_COMMAND${n}=\${TRANSFER_COMMAND${n}:-syncaws}
      eval knumber=2

eval DNCF1="$AWSDIR/NB_DMWF_C08_BUFR/GOES-18/"
eval DNCF2="$AWSDIR/NB_DMWF_C08_BUFR/GOES-19/"

eval SNCF1="NB-?M?F-M?C08_v1r0_g18_s???????????????_e???????????????_c???????????????.bufr"
eval SNCF2="NB-?M?F-M?C08_v1r0_g19_s???????????????_e???????????????_c???????????????.bufr"
